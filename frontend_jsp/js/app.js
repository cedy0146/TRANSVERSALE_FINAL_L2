...
const API = {
  ...
  foyers: {
    ...
    create: (data) =>
      API.post('/foyers', data),

    update: (id, data) =>
      API.put(`/foyers/${id}`, data),

    remove: (id) =>
      API.delete(`/foyers/${id}`)
  },
  ...
  demandes: {
    ...
    update: (id, data) =>
      API.put(`/demandes/${id}`, data),

    remove: (id) =>
      API.delete(`/demandes/${id}`)
  },
  ...
  alloc: {
    ...
    // Lancer allocation
    lancer: () =>
      API.post('/allocation/lancer', {}),

    // Comparer méthodes
    comparer: () =>
      API.get('/allocation/comparer'),

    // Prévision solaire
    previsionSolaire: (historique, options = {}) =>
      API.post('/allocation/prevision-solaire', {
        historique,
        options
      })
  },
...
function initNavigation() {
  ...
  if (user) {
    ...
    if (roleEl) {
      ...
      roleEl.textContent =
        user.role || 'VILLAGEOIS';

      roleEl.className =
        `badge ${user.role === 'RESPONSABLE'
          ? 'badge-blue'
          : 'badge-green'}`;
    }
  }
